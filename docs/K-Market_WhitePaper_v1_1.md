...
PDV svc: 'market' (정규화) → Level 3 서비스 확인
        │
        ▼
market.hondi.net/webapp.html?gwp=1&ctx=감귤...
```

### 10.2 GWP 파라미터

```
https://market.hondi.net/webapp.html
  ?gwp=1
  &gwp_token=HMAC-SHA256-TOKEN
  &svc=kcommerce
  &ctx=감귤%2010kg%20사고%20싶어
  &return=https://hondi.net
```
...
v1.1부터 tax.hondi.net과 연동하여 실시간 세금 납부 현황을 표시한다.

```javascript
// ADMIN_DATA.tax 구조 (tax.hondi.net 연동)
ADMIN_DATA.tax = {
  upcoming: [
...
| `counterpart` | TEXT | `market.hondi.net` |
