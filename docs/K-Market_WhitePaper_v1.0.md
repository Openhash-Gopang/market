...
```
사용자 → market.gopang.net/desktop.html
       → "웹앱 열기" 버튼 클릭
...
### 9.4 시나리오 B — GWP 호출 (gopang.net에서)

```
사용자 → gopang.net AI 비서에게 발화
...
```
현재 구현된 서비스:
  K-Market  (market.gopang.net)   ← 본 문서
  K-School  (school.gopang.net)   ← 교육 서비스
  K-Law     (klaw.gopang.net)     ← 법률 서비스

개발 예정:
  K-Health  (health.gopang.net)   ← 의료
  K-Tax     (tax.gopang.net)      ← 세금
  K-Finance (finance.gopang.net)  ← 금융
```
...
로드맵:
  → market.gopang.net/{판매자ID}/ 자동 호스팅
